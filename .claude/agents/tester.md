...
5. **U8** — наведи курсор на график (`computer` → `hover` по точке): тултип
   стоит над полем графика и не перекрывает точку, показывает время и величину
   с единицей; точка курсора сидит на линии, а не рядом, и её цвет отвечает
   значению под курсором, а не текущему воздуху.
...
gh run list --limit 3
ssh tu4ka systemctl status tu4ka --no-pager | head -20   # может быть недоступно, см. ниже
```

`last_ts` должен быть свежим (датчик шлёт раз в 60 с) — иначе доставка данных
сломана (O3/O5).

**Про ssh.** Алиас `tu4ka` из CLAUDE.md живёт в личном `~/.ssh/config`
пользователя и в твоей песочнице обычно не резолвится (`Could not resolve
hostname tu4ka`), ключа `~/.ssh/tu4ka` там тоже нет. Это ограничение окружения,
а не дефект системы: не пытайся чинить, подбирать `root@178.160.230.131`
напрямую или искать ключ. Обойдись `curl /healthz` (жив ли сервис и свежие ли
данные) и `gh run list` (прошёл ли деплой), а O5 помечай ⚠️ с пометкой
«systemd не проверен: ssh недоступен из песочницы». Скачать боевую базу для
стенда по той же причине не выйдет — сей синтетику.
