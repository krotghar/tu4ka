...
Push защищён HTTP basic auth, креды — на устройство: логин выбирает строку
в таблице `devices`, пароль сверяется с хешем в ней. Переменные
`TU4KA_PUSH_USER`/`TU4KA_PUSH_PASS` остались мостом на случай, когда секрета
в БД нет.
...
- `server/` — python-пакет приложения (`uvicorn server.main:app`):
  ...
  - `auth.py` — креды push и выбор устройства по ним
  ...
  - `quality.py` — качество данных (пока флаг влажности)
...
- Пользователь `tu4ka`, Пароль — `TU4KA_PUSH_PASS` из `/etc/tu4ka/env`
  (его хеш миграция кладёт в `devices.push_secret_sha256`, и дальше сверка
  идёт по нему)
