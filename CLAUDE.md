...
Линтера нет. Тесты — pytest, лежат в `tests/`, 237 штук.
...
- `server/main.py` — всё приложение FastAPI: приём push (`POST /api/v1/push`),
  ...
  `aqi_hi` точен, `aqi_lo` — нижняя оценка) — фронт рисует по ним ленту разброса
  вокруг линии AQI.
...
- `server/static/index.html` — веб-морда одной страницей, без сборки и внешних
  ...
  SVG на клиенте, Chart.js не используется. Шрифты (Manrope, JetBrains Mono)
  вендорены в `server/static/fonts/` (`@font-face`, без Google Fonts CDN).
  Тёмная/светлая тема — `prefers-color-scheme`. AQI-герой с вердиктом и
  рекомендациями, «худшие часы» и сводка за 24 часа считаются на клиенте из
