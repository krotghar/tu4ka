...
не NowCast, иначе тренд на графике смазывается сглаживанием. Плюс `aqi_lo`/
`aqi_hi` — AQI по минимуму/максимуму PM внутри корзины, для ленты разброса на
графике редизайненного дашборда (`aqi_hi` точен, `aqi_lo` — нижняя оценка).
Подробнее про сами окна и корзины — [history-buckets](history-buckets.md).
