...
# --- v3: push_user становится ключом маршрутизации приёма --------------------
#
# В v2 уникален только slug, а push_user — обычная колонка. С приходом
# per-device push по нему выбирается устройство, и две строки с одним логином
# сделали бы выбор недетерминированным (LIMIT 1 без ORDER BY — это «какая
# попадётся»). Уникальность закрывает это на уровне схемы, а не соглашения.
V3 = (
    "CREATE UNIQUE INDEX idx_devices_push_user ON devices(push_user)",
)
